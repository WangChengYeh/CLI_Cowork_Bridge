...
- `cmd` 是 shell pane
- `rd`、`ae`、`qa` 是 agent 名字，也是 pane 标题
...
| 模式 | 示例 |
| :--- | :--- |
| 显式 `/ask` | `/ask ae review the parser changes in src/parser.ts` |
| 显式 `$ask` | `$ask ae review the parser changes in src/parser.ts` |
| 隐式委派 | `让 ae 检查 parser 的边界情况，然后把问题汇总给我。` |

想明确指定目标时，用 `/ask ae ...` 或 `$ask ae ...`。想让当前 agent 自行判断是否委派时，直接用自然语言描述任务。
