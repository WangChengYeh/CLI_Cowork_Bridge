...
- `ae` — Scored quality gate, evaluates the plan using Rubric A (must pass >= 7.0)
...
> Generated by all-plan (`designer` + `inspiration` + `ae`)
...
1. **`designer` Owns the Design**: The `designer` is the sole planner; `inspiration` and `ae` are consultants
