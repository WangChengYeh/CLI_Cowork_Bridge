...
def binding_search_roots(*, workspace_path: Path, project_root: Optional[Path]) -> tuple[Path, ...]:
    roots: list[Path] = []
...
def load_provider_session(*, adapter, provider: str, agent_name: str, roots: tuple[Path, ...], ensure_usable: bool, session_is_usable_fn):
    for root in roots:
        seen_instances: Optional[set[str]] = set()
...
def _candidate_instances(*, provider: str, agent_name: str) -> Optional[tuple[str], ...]:
